...
- **一句话原则。** 如果你的上一条工具调用结果已经确认了操作成功（如 git push 显示成功、文件写入成功），直接确认完成即可。不要反复用不同工具验证同一件事。不要连续调用 TodoWrite。一次汇报、一次停止。
...
- [x] 演示用账号：`罗祎果 / 123456`，预置多道错题，可直接体验全部功能
...
| 用户名 | 密码 | 数据情况 |
|--------|------|----------|
| `罗祎果` | `123456` | 20 道题（高等数学 8/线性代数 6/专业课 4/大学英语 2），多个已掌握知识点 |
